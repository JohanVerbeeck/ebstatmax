...
if(type == "univariate") {
  ...
  ungroup()
	...
	if(matching == "matched"){
		...
		if(side == 1) {
   	 	p_value = pnorm(as.numeric(data_sumf$Z), lower.tail=F)
  		}
  		else if(side == 2){
    		p_value = ifelse(as.numeric(data_sumf$Z)>0,2*pnorm(as.numeric(data_sumf$Z), lower.tail=F),2*pnorm(as.numeric(data_sumf$Z), lower.tail=T)) 
  		}
	} 
